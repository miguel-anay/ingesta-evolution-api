"""
Ingest Chat Images Use Case

Specialized use case for ingesting images from chat messages only.
...
class IngestChatImagesUseCase:
    """
    Use case for ingesting images specifically from chat messages.

    This is a convenience wrapper around IngestImagesUseCase
    that pre-configures it to only process chat messages.
    ...
    async def execute(
        ...
        """
        Execute chat image ingestion for a specific phone number.

        Args:
            numero_celular: Phone number to filter images from (required)
        ...
        """
        logger.info(
            f"Starting chat image ingestion for instance: {instancia}, "
